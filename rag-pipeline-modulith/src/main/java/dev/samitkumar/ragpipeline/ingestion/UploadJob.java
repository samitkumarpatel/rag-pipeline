...
public record UploadJob(
    ...
    public static UploadJob create(@NonNull String originalFilename) {
        return new UploadJob(UUID.randomUUID(), originalFilename, JobStatus.PROCESSING,
    ...
    public UploadJob withStatus(@NonNull JobStatus newStatus) {
        Instant completed = (newStatus == JobStatus.COMPLETED || newStatus == JobStatus.FAILED)
    ...
    public UploadJob withError(@NonNull String error) {
        return new UploadJob(jobId, originalFilename, JobStatus.FAILED, createdAt, Instant.now(),
                files, error);
    }

    // ── Nested types ──────────────────────────────────────────────────────

    public enum JobStatus { PROCESSING, COMPLETED, FAILED }
    ...
    public record FileEntry(
        ...
        public static FileEntry of(@NonNull String originalPath, @NonNull String storedPath,
                                   @NonNull String mimeType, long sizeBytes) {
            return new FileEntry(UUID.randomUUID(), originalPath, storedPath, mimeType,
                    sizeBytes, FileStatus.SAVED);
        }
    }

    public enum FileStatus { SAVED, SKIPPED, QUEUED }
}
